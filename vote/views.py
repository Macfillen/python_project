...
def profile_edit_page(request):
    if request.method == 'POST':
        ...
        history = models.History(date=datetime.datetime.now().date(), time=datetime.datetime.now().time(), user=request.user, log_type=5)
        history.save()
...
def add_claim_page(request, id):
    ...
    if f.is_valid():
        ...
        claim = models.Claim(date=date, user=request.user, comment=f.data['comment'],reason=f.data['reason'], voteid=f.data['voteid'])
        claim.save()
...
#Если еще не голосовал
    if request.method == 'GET':
        ...
        if f.is_valid():
            if poll.multi == 0:
                chosen = []
                chosen.append(f.cleaned_data.get('chosen'))
            else:
...
def vote_add_page(request):
    ...
    context = {}
    context['count_of_options'] = 2
    context['action'] = 'Создание голосования'
    if request.method == 'GET':
        ...
        if f.is_valid():
            ...
            try:
                ...
                m = False
            poll = models.Poll(author = request.user, title = f.data['title'], description = f.data['description'], date = datetime.datetime.now().date(), multi = m)
            poll.save()
